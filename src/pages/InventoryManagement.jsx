  ...
  if (transaction.inventoryType === 'DELIVERY') {
    ...
    // Determine if this is a transfer in or out
    if (isTransfer) {
...
const ProductTransactionsModal = ({ product, transactions, isOpen, onClose, showStockDetails = true, warehouseStocks = [], branchStocks = [] }) => {
  ...
  <div className="mt-6 p-6 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border border-blue-200">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      {!product.warehouseName && !product.branchName && (
        <div className="col-span-4 bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
          <div className="text-gray-500 mb-2">No location-specific stock data available</div>
          <div className="text-sm text-gray-400">
            Viewing all transactions for this product across all locations
          </div>
        </div>
      )}
    </div>
                  ...
                  <div key={idx} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        isInvoiced ? 'bg-green-500' : 'bg-blue-500'
                      }`}>
                        {isInvoiced ? (
                      ...
                      {product.fullTransactionHistory.map((tx, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-start gap-3">
                            <div className="flex flex-col items-center">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                isSubtract ? 'bg-orange-500' : 'bg-green-500'
                              }`}>
                                {isSubtract ? (
  ...
  const EnhancedInventoryManagement = () => {
    ...
    if (inventories && inventories.length > 0) {
      const filtered = inventories.filter(inv => {
        ...
        // Enhanced type matching for transfers
        ...
        if (transactionFilters.type !== 'ALL') {
          if (inv.inventoryType === 'TRANSFER') {
            ...
            if (transactionFilters.type === 'TRANSFER_IN') {
              ...
              matchesType = true; // Show all transfers
            } else {
              matchesType = false;
            }
          } else {
                ...
                filteredWarehouses.map(warehouse => (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-50 ${
                      value === warehouse.warehouseName ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-900'
                    }`}
                  >
              ...
              {filteredBranches.length === 0 ? (
                ...
                filteredBranches.map(branch => (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-50 ${
                      value === branch.branchName ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-900'
                    }`}
                  >
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg border ${
                        stockCurrentPage === warehouseStockTotalPages
                          ? 'text-gray-400 cursor-not-allowed border-gray-200'
                          : 'text-gray-700 hover:bg-gray-50 border-gray-300'
                      }`}
                    >
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg border ${
                        stockCurrentPage === branchStockTotalPages
                          ? 'text-gray-400 cursor-not-allowed border-gray-200'
                          : 'text-gray-700 hover:bg-gray-50 border-gray-300'
                      }`}
                    >
          ...
          <tr key={`transaction-${transaction.id}-${transaction.inventoryType || transaction.transactionType}`} className="hover:bg-gray-50">
            ...
            <td className="px-4 py-3 text-center">
              <button 
                ...
                className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50 rounded transition"
              >
                <Eye size={14} />
                View Details
              </button>
