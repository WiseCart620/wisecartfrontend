...
const SalesSummaryModal = ({ onClose, filterData, statusFilter, searchTerm, companies, branches }) => {
  ...
  const selectedCompany = filterData.companyId ? (companies.find(c => c.id === filterData.companyId)?.companyName || 'All Companies') : 'All Companies';
  const selectedBranch = filterData.branchId ? (branches.find(b => b.id === filterData.branchId)?.branchName || 'All Branches') : 'All Branches';
  ...
  const handleExcel = () => {
    ...
    printRows.forEach((r, i) => rows.push([i + 1, r.productName, r.variationDisplay || '—', r.sku, r.upc, r.quantity, Number(r.unitPrice).toFixed(2), Number(r.amount).toFixed(2)]));
    rows.push([]);
    ...
    a.href = url;
    a.download = `sales_report_${selectedCompany}_${selectedBranch}_${new Date().toISOString().split('T')[0]}.csv`.replace(/[\s/\\:*?"<>|]+/g, '_');
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 flex-shrink-0">
          ...
          <div className="flex gap-2">
            <button onClick={handleExcel} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
              <FileText size={14} /> Excel
            </button>
            <button onClick={handlePrint} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
        ...
        <div className="overflow-auto flex-1 p-5" id="sales-summary-modal-body">
          ...
          <div className="meta" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '4pt', marginBottom: '10pt', fontFamily: 'Arial, sans-serif', fontSize: '7.5pt' }}>
            {[['Company', selectedCompany], ['Branch', selectedBranch], ['Status', statusLabel], ['Date of Coverage', `${dateFrom} to ${dateTo}`]].map(([label, val]) => (
              <div key={label} style={{ border: '0.5pt solid #ccc', padding: '3pt 5pt', borderRadius: '2pt' }}>
                <span className="meta-label" style={{ display: 'block', fontSize: '6.5pt', color: '#666', textTransform: 'uppercase', marginBottom: '1pt' }}>{label}</span>
                {val}
              </div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '7.5pt', fontFamily: 'Arial, sans-serif' }}>
              <thead>
                <tr>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left', whiteSpace: 'nowrap', fontSize: '7pt', width: '30px' }}>#</th>
                ...
                {printRows.map((row, idx) => (
                  <tr key={idx} style={{ background: idx % 2 === 1 ? '#fafafa' : '#fff' }}>
                    ...
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>
                      <div style={{ fontWeight: 600 }}>{row.productName}{row.variationDisplay ? ` — ${row.variationDisplay}` : ''}</div>
                      <div className="sub" style={{ fontSize: '6.5pt', color: '#555' }}>SKU: {row.sku} &nbsp;|&nbsp; UPC: {row.upc}</div>
                    </td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt', textAlign: 'right' }}>{row.quantity.toLocaleString()}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt', textAlign: 'right' }}>{Number(row.unitPrice).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt', textAlign: 'right' }}>{Number(row.amount).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
          ...
          <div className="footer" style={{ marginTop: '8pt', fontSize: '7pt', color: '#666', textAlign: 'right', fontFamily: 'Arial, sans-serif' }}>
...
const SalesManagement = () => {
  ...
  const handleDownloadReport = useCallback(async () => {
    ...
    try {
      ...
      // Aggregate by product+variation (same logic as print)
      const productMap = new Map();
      allSales.forEach(sale => {
        (sale.items || []).forEach(item => {
          const sku = item.variation?.sku || item.product?.sku || '—';
      ...
      toast.success(`Report downloaded! ${productRows.length} products · ${grandQty.toLocaleString()} units`, { id: toastId });
    } catch (error) {
      console.error('Download report error:', error);
      toast.error('Failed to generate report. Please try again.', { id: toastId });
    }
  }, [filterData, statusFilter, searchTerm, companies, branches]);
                ...
                <div className="p-8 border-t border-gray-200 flex justify-end gap-3 print:hidden sticky bottom-0 bg-white rounded-b-2xl">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const content = document.getElementById('invoice-report').innerHTML;
                      const w = window.open('', '_blank');
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    input { border: none; background: transparent; outline: none; padding: 0; margin: 0; }
    input[type="checkbox"] { display: none; }
    input[readonly] { width: 100%; text-align: right; line-height: 1.2; }
    ...
    .font-bold { font-weight: bold; color: transparent; }
    .font-semibold { font-weight: 600; color: transparent; }
    .font-medium { font-weight: 500; color: transparent; }
    .font-serif { font-family: serif; }
    .text-gray-900 { color: black; }
    .print-visible { color: black !important; }
    .print-visible * { color: black !important; }
    .w-full { width: 100%; }
    .w-48 { width: 12rem; }
    .w-36 { width: 9rem; }
    ...
    div.print-visible { color: black; }
    span.print-visible { color: black; }
    td { color: black; }
    th { color: black; }
...
<body>
  <div style="transform: scale(0.90); transform-origin: top left; width: 111.11%;">
    ${content}
  </div>
</body>
</html>`);
