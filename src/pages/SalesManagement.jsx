...
const SalesManagement = () => {
  ...
  const handleDownloadReport = useCallback(async () => {
    ...
    try {
      ...
      if (allSales.length === 0) {
        toast.error('No sales data found for the selected filters.', { id: toastId });
        return;
      }

      const selectedCompany = filterData.companyId
        ? (companies.find(c => c.id === filterData.companyId)?.companyName
          || companies.find(c => c.id === filterData.companyId)?.name
          || 'Unknown')
        : 'All Companies';

      const selectedBranch = filterData.branchId
        ? (branches.find(b => b.id === filterData.branchId)?.branchName || 'Unknown')
        : 'All Branches';
      ...
      allSales.forEach(sale => {
        ...
        const branchCode = sale.branch?.branchCode || '';
        const companyName = sale.company?.companyName || '';
        const period = `${months[sale.month - 1]} ${sale.year}`;
        const encodedBy = sale.createdBy || sale.generatedBy || '—';
      ...
      const totalQty = allSales.reduce(
        (sum, s) => sum + (s.items || []).reduce((q, i) => q + (i.quantity || 0), 0), 0
      );
      ...
      a.href = url;
      a.download = `sales_report_${selectedCompany}_${selectedBranch}_${new Date().toISOString().split('T')[0]}.csv`
        .replace(/[\s/\\:*?"<>|]+/g, '_');
      ...
      toast.success(
        `Report downloaded! ${allSales.length} sales · ${totalQty.toLocaleString()} units`,
        { id: toastId }
      );
    } catch (error) {
  ...
  }, [filterData, statusFilter, searchTerm, companies, branches, months]);
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1 rounded-lg border-2 border-green-600 text-green-700 hover:bg-green-50 text-xs font-semibold transition-colors"
                          title="Download all filtered sales as CSV"
                        >
                          <FileText size={13} />
                          Download Report
                        </button>
                        <button
                          onClick={async () => {
                            const toastId = toast.loading('Preparing print report...');
                            try {
                              const startDateObj = filterData.startDate ? new Date(filterData.startDate) : null;
                              const endDateObj = filterData.endDate ? new Date(filterData.endDate) : null;
                              ...
                              // Flatten all sales into product rows then aggregate by product+variation
                              ...
                              // Build the summary print content
                              const selectedCompany = filterData.companyId
                              ...
                              const grandTotal = productRows.reduce((sum, r) => sum + r.amount, 0);
                              const grandQty = productRows.reduce((sum, r) => sum + r.quantity, 0);

                              const printEl = document.getElementById('sales-summary-print');
                              printEl.innerHTML = `
      <div style="font-family: Arial, sans-serif; font-size: 9pt;">
        <table style="width:100%; border-collapse:collapse; font-size:7.5pt;">
          <thead>
            <tr>
              <th style="border:0.5pt solid #999; padding:3pt 4pt; background:#f0f0f0; text-align:left; white-space:nowrap;">#</th>
              <th style="border:0.5pt solid #999; padding:3pt 4pt; background:#f0f0f0; text-align:left;">PRODUCT / SKU / UPC</th>
              <th style="border:0.5pt solid #999; padding:3pt 4pt; background:#f0f0f0; text-align:right;">QTY</th>
              <th style="border:0.5pt solid #999; padding:3pt 4pt; background:#f0f0f0; text-align:right;">UNIT PRICE</th>
              <th style="border:0.5pt solid #999; padding:3pt 4pt; background:#f0f0f0; text-align:right;">TOTAL</th>
            </tr>
          </thead>
          <tbody>
            ${productRows.map((row, idx) => `
              <tr style="background:${idx % 2 === 1 ? '#fafafa' : '#fff'};">
                ...
                <td style="border:0.5pt solid #ccc; padding:2.5pt 4pt;">
                  <div style="font-weight:600;">${row.productName}${row.variationDisplay ? ` — ${row.variationDisplay}` : ''}</div>
                  <div style="font-size:6.5pt; color:#555;">SKU: ${row.sku} &nbsp;|&nbsp; UPC: ${row.upc}</div>
                </td>
                <td style="border:0.5pt solid #ccc; padding:2.5pt 4pt; text-align:right;">${row.quantity.toLocaleString()}</td>
                <td style="border:0.5pt solid #ccc; padding:2.5pt 4pt; text-align:right;">${Number(row.unitPrice).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                <td style="border:0.5pt solid #ccc; padding:2.5pt 4pt; text-align:right;">${Number(row.amount).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
            `).join('')}
          </tbody>
          <tfoot>
            <tr>
              <td colspan="2" style="border:0.5pt solid #ccc; padding:3pt 4pt; font-weight:bold; text-align:right; border-top:1.5pt solid #333;">GRAND TOTAL</td>
              <td style="border:0.5pt solid #ccc; padding:3pt 4pt; font-weight:bold; text-align:right; border-top:1.5pt solid #333;">${grandQty.toLocaleString()}</td>
              <td style="border:0.5pt solid #ccc; padding:3pt 4pt; border-top:1.5pt solid #333;"></td>
              <td style="border:0.5pt solid #ccc; padding:3pt 4pt; font-weight:bold; text-align:right; border-top:1.5pt solid #333;">₱${grandTotal.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
            </tr>
          </tfoot>
        </table>
        <div style="margin-top:6pt; font-size:7pt; color:#666; text-align:right;">
          Generated: ${new Date().toLocaleString('en-PH', { timeZone: 'Asia/Manila' })} &nbsp;|&nbsp; Total Products: ${productRows.length}
        </div>
      </div>
    `;
                              ...
                              console.error(err);
                              toast.error('Failed to prepare print report', { id: toastId });
                            }
                          }}
                          className="flex items-center gap-1.5 px-3 py-1 rounded-lg border-2 border-gray-500 text-gray-700 hover:bg-gray-50 text-xs font-semibold transition-colors"
                          title="Print current page as A4"
                        >
                          <Printer size={13} />
                          Print
                        </button>
        ...
        <div id="sales-print-area">
          {/* Print-only header */}
          <div className="sales-print-header hidden">
            <h1>Sales Report — WISECART MERCHANTS CORP.</h1>
          </div>
          <div className="sales-print-meta hidden">
            <div className="sales-print-meta-item">
            ...
            <div className="sales-print-meta-item">
              ...
              {filterData.startDate || '—'} to {filterData.endDate || '—'}
            </div>
          </div>

          {/* Print-only summary report */}
          <div id="sales-summary-print" className="hidden">
            <table className="w-full border-collapse text-xs" style={{ fontFamily: 'Arial, sans-serif' }}>
              <thead>
                <tr>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>#</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Branch</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Branch Code</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Company</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Period</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Encoded By</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'right' }}>Total Amount</th>
                  <th style={{ border: '0.5pt solid #999', padding: '3pt 4pt', background: '#f0f0f0', textAlign: 'left' }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {currentSales.map((sale, idx) => (
                  <tr key={sale.id}>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{((currentPage - 1) * 10) + idx + 1}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{sale.branch.branchName}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{sale.branch.branchCode}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{sale.company.companyName}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{months[sale.month - 1]} {sale.year}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{sale.createdBy || sale.generatedBy || '—'}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt', textAlign: 'right' }}>{formatCurrency(sale.totalAmount)}</td>
                    <td style={{ border: '0.5pt solid #ccc', padding: '2.5pt 4pt' }}>{sale.status}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  <td colSpan={6} style={{ border: '0.5pt solid #ccc', padding: '3pt 4pt', fontWeight: 'bold', textAlign: 'right', borderTop: '1.5pt solid #333' }}>
                    GRAND TOTAL ({currentSales.length} sales):
                  </td>
                ...
                <tr>
                  <td colSpan={8} style={{ padding: '4pt', fontSize: '7pt', color: '#666', textAlign: 'right', borderTop: '0.5pt solid #ccc' }}>
                    Pending: ₱{(allFilteredSales.pendingAmount || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })} &nbsp;|&nbsp;
                    Confirmed: ₱{(allFilteredSales.confirmedAmount || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })} &nbsp;|&nbsp;
                    Invoiced: ₱{(allFilteredSales.invoicedAmount || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                  </td>
                </tr>
              </tfoot>
            </table>
